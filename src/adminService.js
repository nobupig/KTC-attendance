...
function getAdminMissingSummaryRangeGrouped(startDate, endDate, gradeFilter) {
  if (!canAccessAdminPage()) {
    throw new Error('教務画面の権限がありません。');
  }
  ...
  if (!startYmd || !endYmd) {
    throw new Error('開始日・終了日を指定してください。');
  }
  ...
  classSessionsData.rows.forEach(function(row) {
    const ymd = formatDateToYmd(row[csCol.date]);
    ...
    if (gradeFilter && String(classInfo.grade || '') !== String(gradeFilter)) return;
    ...
    if (!grouped[groupKey]) {
      grouped[groupKey] = {
        subjectName: classInfo.subjectName || '(科目名未設定)',
        teacherName: teacherLabel || '(担当者未設定)',
    ...
    if (!grouped[groupKey].dates[ymd]) {
      grouped[groupKey].dates[ymd] = [];
    }

    grouped[groupKey].dates[ymd].push(Number(period));
  });

  const rows = Object.keys(grouped).map(function(key) {
    const item = grouped[key];

    const dateTexts = Object.keys(item.dates).sort().map(function(ymd) {
      const periods = item.dates[ymd]
        .filter(function(p) { return !isNaN(p); })
        .sort(function(a, b) { return a - b; });

      return formatAdminDateForDisplay_(ymd) + ' ' + formatAdminPeriodRange_(periods);
    });
    ...
    return {
      ...
      grade: item.grade,
      unit: item.unit,
      missingText: dateTexts.join('、')
    };
  });

  rows.sort(function(a, b) {
    if (String(a.grade) !== String(b.grade)) return Number(a.grade) - Number(b.grade);
    if (String(a.unit) !== String(b.unit)) return String(a.unit).localeCompare(String(b.unit), 'ja');
    if (a.subjectName !== b.subjectName) return a.subjectName.localeCompare(b.subjectName, 'ja');
    return a.teacherName.localeCompare(b.teacherName, 'ja');
  });

  const copyText = buildAdminMissingGroupedCopyText_(startYmd, endYmd, rows, gradeFilter);
...
function buildAdminTeacherSummaryLabel_(classId, teachers) {
  const list = Array.isArray(teachers) ? teachers : [];
  if (!list.length) return '';

  const main = list.find(function(t) {
    return String(t.roleType || '').toLowerCase() === 'main';
  }) || list[0];

  const mainName = main && main.teacherName ? main.teacherName : '';

  if (!mainName) return '';

  if (String(classId || '').indexOf('工学実験') !== -1 && list.length > 1) {
    return mainName + ' 他';
  }
...
function buildAdminMissingGroupedCopyText_(startYmd, endYmd, rows, gradeFilter) {
  const gradeText = gradeFilter ? gradeFilter + '年 ' : '';
